...
class KmeansHomogenTableSuite extends FunctionsSuite with Logging {
    ...
    test("test compute Kmeans HomogenTable") {
        ...
        val centroids = kmeansDAL.cKMeansOneapiComputeWithInitCenters(dataTable.getcObejct(), sourceData.length, sourceData(0).length,
          centroidsTable.getcObejct(), centroidData.length,10, 0.001, 5, 1, 1, TestCommon.getComputeDevice.ordinal(), gpuIndices, result);
